...
SpeedTest::SpeedTest(float minServerVersion) : mLatency(0), mUploadSpeed(0), mDownloadSpeed(0)
{
...
bool SpeedTest::share(const ServerInfo& server, std::string& image_url)
{
    ...
    hash << std::setprecision(0) << std::fixed << mLatency << "-" << std::setprecision(2) << std::fixed
         << (mUploadSpeed * 1000) << "-" << std::setprecision(2) << std::fixed << (mDownloadSpeed * 1000) << "-"
         << SPEED_TEST_API_KEY;
